...
def train_unet2(read_from=None, is_shuffle=True, is_test=False, ntest=0,
    ...
    #    nlayers = 2
    #    filter_size = 3
    new_approx = _approxnode.create_unet(nlayers=nlayers, feature_base=nfeatures, filter_size=filter_size, from_queue=True, in_batch=lo_imgs, out_batch=lo_grounds, is_const_bias=is_const_bias, const_std=const_std, passthrough_op='plus')
            ...
            for i in range(ntest // batch_size):
                for (grounds, unet_outputs, loss, input_imgs) in [(lo_grounds, lo_output, lo_loss, inputs[0]), (med_grounds, med_output, med_loss, inputs[1]), (hi_grounds, hi_output, hi_loss, inputs[2])]:
                    ...
                    for j in range(batch_size):
                        ...
                        #skimage.io.imsave('test_output6/'+str(count)+'ground.png', numpy.clip(output_ground, 0.0, 1.0))
                        #skimage.io.imsave('test_output6/'+str(count)+'out3.png', numpy.clip(output_img, 0.0, 1.0))
                        #skimage.io.imsave('test_output6/'+str(count)+'ainput.png', numpy.clip(input_img, 0.0, 1.0))
...
if __name__ == '__main__':
    ...
    if saver_name == 'saver56':
        ...
        app_name = '_f2'
        in_filtersize = 5
    elif saver_name == 'saver66':
        # somehow works, a bit less bright
        in_loss = 'L1+L2'
        in_scale = 2.0
        loss_crop = False
        app_name = '_test_sharpen_small'
    elif saver_name == 'saver67':
        in_loss = 'L2_slope'
        loss_crop = False
        app_name = '_test_sharpen_small'
    elif saver_name == 'saver68':
        # somehow works, a bit less bright
        in_loss = 'L1+L2'
        in_scale = 0.5
        loss_crop = False
        app_name = '_test_sharpen_small'
    elif saver_name == 'saver69':
        # much less brighter
        in_loss = 'L2'
        loss_crop = False
        app_name = '_test_sharpen_small'
    elif saver_name == 'saver70':
        # much less brighter
        in_loss = 'L2'
        loss_crop = False
        app_name = '_test_sharpen_small'
        in_nlayers = 1
    elif saver_name == 'saver71':
        # failed
        in_loss = 'L1+L2'
        in_scale = 0.5
        loss_crop = False
        ...
        in_activation = 'relu'
        in_lrate = 0.01
    elif saver_name == 'saver76':
        # a bit less brighter, 1 in 10
        in_loss = 'L2_slope'
        loss_crop = False
        app_name = '_test_sharpen_small'
        in_nlayers = 1
        in_activation = 'relu'
        in_lrate = 0.01
    elif saver_name == 'saver79':
        # slight color change, especially for pure red
        # 1 in 10
        in_loss = 'L2'
        loss_crop = False
        ...
        in_activation = 'relu'
        in_filtersize = 5
    elif saver_name == 'saver80':
        # moderate color change, 2 in 10
        in_loss = 'L2'
        loss_crop = False
        app_name = '_test_sharpen_large'
        in_nlayers = 5
        in_activation = 'relu'
    elif saver_name == 'saver81':
        # very slight color change, 1 in 10
        in_loss = 'L2_slope'
        loss_crop = False
        app_name = '_test_sharpen_large'
        in_nlayers = 5
        ...
        in_activation = 'relu'
        in_filtersize = 5
    elif saver_name == 'saver83':
        # slight color change, 1 in 10
        in_loss = 'L1+L2'
        in_scale = 0.5
        loss_crop = False
        app_name = '_test_sharpen_large'
        ...
        in_filtersize = 5
        in_nfeatures = 6
    elif saver_name == 'saver85':
        # moderate color change, 1 in 10
        in_loss = 'L2'
        loss_crop = False
        app_name = '_test_sharpen_large'
        in_nlayers = 5
        ...
        in_nfeatures = 5
        in_lrate = 0.01
    elif saver_name == 'saver86':
        # slight color change, 4 in 10
        in_loss = 'L1+L2'
        in_scale = 0.5
        loss_crop = False
        app_name = '_test_sharpen_large'
