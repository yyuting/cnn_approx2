...
class ApproxNode:
    ...
    def build_graph(self, nlayers=2,
        ...
        for layer in range(nlayers):
            weights['down_conv'+str(layer)+'w'] = tf.Variable(tf.truncated_normal([filter_size, filter_size, 1, 1], stddev=0.1))
            weights['up_conv'+str(layer)+'w'] = tf.Variable(tf.truncated_normal([filter_size, filter_size, 1, 1], stddev=0.1))
            weights['passthrough'+str(layer)+'w'] = tf.Variable(tf.truncated_normal([filter_size, filter_size, 1, 1], stddev=0.1))
            weights['deconv'+str(layer)+'w'] = tf.Variable(tf.truncated_normal([pool_size, pool_size, 1, 1], stddev=0.1))
            weights['down_conv'+str(layer)+'b'] = tf.Variable(tf.zeros([]))
            weights['up_conv'+str(layer)+'b'] = tf.Variable(tf.zeros([]))
            weights['passthrough'+str(layer)+'b'] = tf.Variable(tf.zeros([]))
            weights['deconv'+str(layer)+'b'] = tf.Variable(tf.zeros([]))
        
        outputs = []
        for channel in range(3):
            in_node = self.in_pl[:, :, :, channel]
            in_node = tf.expand_dims(in_node, 3)
            down_pyramid = []
            for layer in range(nlayers):
                w1 = weights['down_conv'+str(layer)+'w']
                b1 = weights['down_conv'+str(layer)+'b']
                conv1 = conv2d(in_node, w1, b1)
                w2 = weights['passthrough'+str(layer)+'w']
                b2 = weights['passthrough'+str(layer)+'b']
            ...
            for layer in range(nlayers-2, -1, -1):
                w_deconv = weights['deconv'+str(layer)+'w']
                b_deconv = weights['deconv'+str(layer)+'b']
                ...
                passthrough = deconv + down_pyramid[layer]
                w1 = weights['up_conv'+str(layer)+'w']
                b1 = weights['up_conv'+str(layer)+'b']
                conv1 = conv2d(passthrough, w1, b1)
                in_node = conv1
            outputs.append(tf.squeeze(in_node, [3]))
...
def main(args):
    ...
    if saver_name == 'saver147':
        kw['loss_func'] = 'L2'
        app_name = '_test_blur_large'
        kw['nn_kw']['nlayers'] = 5
        kw['nn_kw']['filter_size'] = 5
    elif saver_name == 'saver151':
        kw['loss_func'] = 'L2'
        app_name = '_test_blur_large'
        kw['nn_kw']['nlayers'] = 5
        kw['nn_kw']['filter_size'] = 5
        kw['minimizer_kw']['lrate'] = 0.01
        kw['train_kw']['iters'] = 100000
        kw['train_kw']['tolerance'] = 50
