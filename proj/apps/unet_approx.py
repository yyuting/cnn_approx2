    ...
    def build_graph(self, nlayers=2,
        ...
        for layer in range(nlayers-2, -1, -1):
            ...
            if not less_conv:
                ...
                w2 = get_weight([filter_size, filter_size, prev_features, prev_features], std, w2_name)
                b2 = get_bias([prev_features], std, w2_name)
                conv2 = conv2d(conv1, w2, b2)
                size_diff += filter_size // 2
            else:
                conv2 = conv1
        ...
        if no_out_layer:
            ...
            out_name = 'output'
            w_out = get_weight([1, 1, nfeatures, 3], std, out_name)
            b_out = get_bias([3], std, out_name)
            output = conv2d(in_node, w_out, b_out)
...
def main(args):
    ...
    if saver_name == 'saver131':
        kw['loss_func'] = 'L1+L2'
        kw['loss_kw']['scale'] = 2.0
        app_name = '_test_blur_large'
        kw['nn_kw']['nlayers'] = 5
        kw['nn_kw']['filter_size'] = 5
    elif saver_name == 'saver132':
        kw['loss_func'] = 'L2'
        app_name = '_test_blur_large'
        kw['nn_kw']['nlayers'] = 5
        kw['nn_kw']['filter_size'] = 5
    elif saver_name == 'saver133':
        # test1 on timing
        kw['loss_func'] = 'L2'
        app_name = '_test_blur_large'
        kw['nn_kw']['nlayers'] = 1
        kw['nn_kw']['filter_size'] = 33
        kw['nn_kw']['less_conv'] = True
        kw['nn_kw']['no_out_layer'] = True
    elif saver_name == 'saver134':
        # test2 on timing
        kw['loss_func'] = 'L2'
        app_name = '_test_blur_large'
        kw['nn_kw']['nlayers'] = 1
        kw['nn_kw']['filter_size'] = 33
        kw['nn_kw']['no_out_layer'] = True
    elif saver_name == 'saver135':
        # failed
        kw['loss_func'] = 'L2'
        app_name = '_test_blur_large'
        kw['nn_kw']['nlayers'] = 5
        kw['nn_kw']['filter_size'] = 5
    elif saver_name == 'saver137':
        kw['loss_func'] = 'L2'
        app_name = '_test_blur_large'
        kw['nn_kw']['nlayers'] = 5
        kw['nn_kw']['filter_size'] = 5
        kw['nn_kw']['upsample'] = 'nn'
