    ...
    if not is_test:
        for k in range(restarts):
            ...
            for i in range(10000):
                ...
                if i % 50 == 0:
                    ...
                    print(i)
                    validate_loss = numpy.mean(sess.run([losses[3], losses[4], losses[5]], feed_dict={keep_prob: 1.0}))
        ...
        for i in range(ntest // batch_size):
            for (grounds, unet_outputs, loss, input_imgs) in [(lo_grounds, lo_output, lo_loss, inputs[0]), (med_grounds, med_output, med_loss, inputs[1]), (hi_grounds, hi_output, hi_loss, inputs[2])]:
                ground_vals, unet_vals, loss_val, input_vals = sess.run([grounds, unet_outputs, loss, input_imgs], feed_dict={keep_prob: 1.0})
                print(loss_val)
                for j in range(batch_size):
                    output_ground = numpy.squeeze(ground_vals[j, :, :, :])
                    output_img = numpy.squeeze(unet_vals[j, :, :, :])
                    input_img = numpy.squeeze(input_vals[j, :, :, :])
                    skimage.io.imsave('test_output2/'+str(count)+'ground.png', numpy.clip(output_ground, 0.0, 1.0))
                    skimage.io.imsave('test_output2/'+str(count)+'out3.png', numpy.clip(output_img, 0.0, 1.0))
                    #skimage.io.imsave('test_output2/'+str(count)+'ainput.png', numpy.clip(input_img, 0.0, 1.0))
...
if __name__ == '__main__':
    ...
    if saver_name == 'saver23':
        # good result with good boundary
        in_batch = [40, 20, 10]
        in_loss = 'L2_slope'
        in_delta = 0.3
    elif saver_name == 'saver24':
        # good enough, 1 in 10
        in_batch = [10, 10, 10]
        in_loss = 'L2_slope'
        in_delta = 0.3
    elif saver_name == 'saver28':
        # failed
        in_batch = [40, 20, 10]
        in_loss = 'L2_slope'
        in_delta = 0.3
        ...
        in_nlayers = 5
        app_name = '_f2'
    elif saver_name == 'saver32':
        # good enough, 1 in 10
        in_batch = [40, 20, 10]
        in_loss = 'L2_slope'
        in_delta = 0.3
    elif saver_name == 'saver33':
        # 1 in 10
        in_batch = [160, 40, 10]
        in_loss = 'L2_slope'
        in_delta = 0.3
        loss_crop = False
    elif saver_name == 'saver34':
        # failed
        in_batch = [160, 40, 10]
        in_loss = 'L1+L2'
        loss_crop = False
    elif saver_name == 'saver41':
        # approx 1 in 20 or even 1 in 30
        in_batch = [160, 40, 10]
        in_loss = 'L1+L2'
        loss_crop = False
        in_scale = 0.5
    elif saver_name == 'saver42':
        # 1 in 10
        in_batch = [160, 40, 10]
        in_loss = 'L1+L2'
        loss_crop = False
        in_scale = 2.0
    elif saver_name == 'saver50':
        in_batch = [160, 40, 10]
        in_loss = 'L1+L2'
        loss_crop = False
        in_scale = 2.0
        in_use_dropout = True
    elif saver_name == 'saver55':
        in_batch = [40, 20, 10]
        in_loss = 'L1+L2'
        in_scale = 2.0
        loss_crop = False
